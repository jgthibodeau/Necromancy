	...
	public class AiEditor : Editor {
		...
		public override void OnInspectorGUI()
		{
			...
			if(_showMain){
				...
				ai._IsEnemy = EditorGUILayout.Toggle("Is Enemy", ai._IsEnemy, "Toggle");
			...
			if(_showSpeeds){
				...
				if(ai._CanFollowPlayer){
					ai.followSpeed = EditorGUILayout.Slider("Follow Speed", ai.followSpeed,0.0f,10000.0f);
				}
				if(ai._CanWander || ai._CanPatrol){
					ai.wanderSpeed = EditorGUILayout.Slider("Wander Speed",ai.wanderSpeed,0.0f,10000.0f);
				}
				if(ai._CanPatrol){
					ai.patrolSpeed = EditorGUILayout.Slider("Patrol Speed", ai.patrolSpeed,0.0f,10000.0f);
				}
				ai.rotationSpeed = EditorGUILayout.Slider("Rotation Speed", ai.rotationSpeed,0.0f,1000.0f);
				ai.gravity = EditorGUILayout.Slider("Gravity", ai.gravity,0.0f,1000.0f);
				ai.drag = EditorGUILayout.Slider("Drag", ai.drag,0.0f,1.0f);
			} else {
			...
			if(ai._HasAvoidance){
				...
				if(_showAvoid){
					...
					ai.avoidSpeed = EditorGUILayout.Slider("Avoidance Speed", ai.avoidSpeed,0.01f,5000.0f);
