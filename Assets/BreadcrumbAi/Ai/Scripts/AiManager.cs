...
namespace BreadcrumbAi{
	public static class AiManager {
		...
		static public GameObject[] players, breadcrumbs, waypoints, enemies, friendlies, allFriendlies;
        ...
        public static IEnumerator Ai_Lists(this Ai ai){
			while(ai.enabled){
				try{
					...
					friendlies = GameObject.FindGameObjectsWithTag(friendlyString);

					allFriendlies = new GameObject[players.Length + friendlies.Length];
					players.CopyTo(allFriendlies, 0);
					friendlies.CopyTo(allFriendlies, players.Length);
//
//					Debug.Log(friendlies.Length + " friendlies");
//					Debug.Log(allFriendlies.Length + " all friendlies");
//					int i = 0;
//					foreach(GameObject go in allFriendlies) {
//						Debug.Log((i++) + " " + go);
//					}
		...
		public static IEnumerator Ai_Layers(this Ai ai){
			...
			ai.friendlyLayer = (1 << LayerMask.NameToLayer(friendlyString));
		...
		*/
		...
		public static Transform Ai_FindTarget(this Ai ai) {
			if(ai.Target){
				Debug.Log ("Checking on current target");
				if(IsValidTarget(ai, ai.Target)){
					Debug.Log ("Checking line of sight " + ai.Target);
					if(Physics.Linecast(ai.transform.position,ai.Target.position, out hit, GetTargetLayerMask(ai))){
						Debug.Log ("Checking line of sight is " + ai.Target +", " + hit.collider.transform.root);
						if(hit.collider.transform.root == ai.Target.transform){
							return ai.Target;
						} else {
							ai.Target = null;
						}
			...
			float distance = Mathf.Infinity;
			GameObject[] targetsToCheck;
			if (ai._IsEnemy) {
				Debug.Log ("Finding new target using allFriendlies");
				targetsToCheck = allFriendlies;
			} else {
				Debug.Log ("Finding new target using enemies");
				targetsToCheck = enemies;
			}
			if(targetsToCheck != null){
				foreach(GameObject t in targetsToCheck){
					Debug.Log ("Checking " + t);
					Debug.DrawLine (ai.transform.position, t.transform.position, Color.blue);
					Debug.DrawRay (ai.transform.position, (t.transform.position - ai.transform.position) * ai.visionDistance, Color.white);
					if(IsValidTarget(ai, t.transform)){
						Debug.Log ("Checking line of sight " + t);
						if(Physics.Linecast(ai.transform.position, t.transform.position, out hit, GetTargetLayerMask(ai))){
							Debug.Log ("Checking line of sight is " + t +", " + hit.collider.transform.root);
							if(hit.collider.transform.root == t.transform){
								Vector3 diff = t.transform.position - ai.transform.position;
								float curDistance = diff.sqrMagnitude;
								if (curDistance < distance){
									ai.Target = t.transform;
									ai.FollowingTarget = t.transform;
									distance = curDistance;
								}
							}
						}
		...
		private static int GetTargetLayerMask(Ai ai) {
			int layer = ai.breadcrumbLayer | ai.waypointLayer;
			if (ai._IsEnemy) {
				layer |= ai.enemyLayer;
			} else {
				layer |= ai.friendlyLayer;
			}
			return ~layer;
		}

		private static bool IsValidTarget(Ai ai, Transform target) {
			return ai.InRange (target.position, ai.visionDistance)
			&& !target.GetComponent<Health> ().IsDead ()
			&& (ai.gameObject.tag != target.tag);
		}
		...
		*/
		...
		public static Transform Ai_FindAnyAi(this Ai ai){
			if(ai.FollowingAi){
				if(ai.InRange(ai.FollowingAi.position, ai.visionDistance)){
					if(Physics.Linecast(ai.transform.position, ai.FollowingAi.position, out hit, ~(ai.breadcrumbLayer | ai.waypointLayer))){
